...
#     6. Support R lang.
#     7. Support resuming failed workflows within Argo Workflows.
#     8. Support gang-scheduled clusters for distributed PyTorch/TF.
#     9. Make production-tokens optional.
#
#
    ...
    def _dag_templates(self):
        def _visit(node, exit_node=None, templates=[], dag_tasks=[]):
            ...
            if node.name == "start":
                ...
                dag_task = (
                    DAGTask(node.name).dependencies(node.in_funcs).template(node.name)
                )
            elif (
            ...
            if node.type == "end":
                return [
                    Template(self.name).dag(DAGTemplate().fail_fast().tasks(dag_tasks))
                ] + templates, dag_tasks
            # Emit if we have reached the end of the sub workflow
            elif exit_node is not None and exit_node in node.out_funcs:
                return templates, dag_tasks
            # For linear nodes continue traversing to the next node
            if node.type in ("start", "linear", "join"):
                return _visit(
            ...
            # For foreach nodes generate a new sub DAGTemplate
                ...
                foreach_template_name = "%s-foreach-%s" % (
                    node.name,
                    node.foreach_param,
                )
                foreach_task = (
                    ...
                    .with_param(
                        "{{tasks.%s.outputs.parameters.num-splits}}" % node.name
                    )
                )
                ...
                join_foreach_task = (
                    DAGTask(self.graph[node.matching_join].name)
    ...
    def _container_templates(self):
        ...
        for node in self.graph:
            ...
            step = [
                ...
                # This is where the Metaflow state transfer magic happens
            ...
            outputs = [Parameter("task-id").value(task_id)]
            if node.type == "foreach":
                # Emit split cardinality from foreach task
                outputs.append(
                    Parameter("num-splits").valueFrom({"path": "/tmp/splits"})
                )
